...
- Ephemeral-guardian CI mechanism (committed, working): the `solana-e2e` job in `ci.yml` generates an ephemeral guardian keypair, rewrites the `GUARDIAN_PUBKEY` constant in `programs/succra/src/lib.rs` in-container only, and exports `SUCCRA_TEST_GUARDIAN_PATH` for the mocha quarantine suite. Acceptable because the patch never leaves the container (same class as `anchor keys sync`), the secret is never logged, and the quarantine logic exercised is identical. Coverage gap: CI never exercises the production `GUARDIAN_PUBKEY` constant — revisit in Phase 9 hardening.
